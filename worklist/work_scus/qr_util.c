 ...
 */
static char id_string[] = "$Header: /usr/adac/Repository/camera/camera/control/src/worklist/work_scus/qr_util.c,v 1.1 Exp $";
...
**
** NAME
**    PrintErrorMessage - Print error messages and tracing information
**
** ARGUMENTS
**    A_prefix          char *    Prefix to error message
**    A_function        char *    Name of function error occured in or during
**    A_status          int       Status returned by a library routine
**    A_errorMessage    char *    Extra error message
**
** DESCRIPTION
**    Routine is used to print out error messages that consist of the
**    function name that the error occured in, the specific routine that 
**    was being done, the status returned from a library call, and a
**    extra message that may be needed.
**
** RETURNS
**    none
**
...
void PrintErrorMessage ( char *A_prefix, char *A_function,
                         int A_status, char *A_errorMessage )
{
    ...
    {
       printf ( "***\t%s: %s\n", A_prefix,
                 MC_Error_Message ( (MC_STATUS)A_status ) );
    }
    else
    {
       printf ( "***\t%s: %s\n", A_prefix, A_errorMessage );
    }
...
**
*****************************************************************************/
void WriteToListFile ( int A_messageID, char* A_outFileName )
{
#ifdef LIST_MESSAGES

    static char S_prefix[]="WriteToListFile";
#if !defined(_WIN32)
    FILE *outfile;
...
**
*****************************************************************************/
void ValMessage(int A_messageID, char* A_name)
{
    ...
    {
        printf("%s message validated!\n", A_name); 
    }
    else if (status == MC_DOES_NOT_VALIDATE)
    {
        printf ( "\n***\tVALIDATION ERROR: %s\n", A_name );
        printf("(Grp ,Elmt)  Value# Type  Message\n");
        do
        {
    ...
    {
        PrintErrorMessage ( "ValMessage", "MC_Validate_Message", status, NULL);
    }   
#endif
    return; 
} /* ValMessages */
...
**
** RETURNS
**    SUCCESS if the function finishes properly.
**    FAILURE if the function detects an error.
**
*****************************************************************************/
int
LLCreate(
        ...
        )
{
    ...
    */
    old_node->data_ptr = malloc( list_ptr->node_size );
    ...
    */
    memcpy( old_node->data_ptr, data, list_ptr->node_size );
         ...
         )
{
    ...
    ** Get the data
    */
    memcpy( data, list_ptr->node_ptr->data_ptr, list_ptr->node_size );
      ...
      )
{
    ...
    */
    dummy_item = malloc( list_ptr->node_size );
        ...
        )
{
    ...
    */
    memcpy( new_node, new_data, list_ptr->node_size );
...
**
** NAME
**    LogError - This functions displays a generic error message with
**               optional other data.
**
** ARGUMENTS
**    error_level   int    The type of error that was encountered.  There are
**                         3 defines:  ERROR_LVL_[1,2,3].  These correspond to
**                         program errors, warnings, or messages.
**    function_name char * A pointer to the function name where the error was
**                         encountered.
**    line_number   int    The source code line number associated with the
**                         error - or a pre-defined error code number.
**    status    MC_STATUS  The MergeCom-3 status of a function.
**    format        char * A pointer to a format string.
**    ...                  other function parameters.
**
** DESCRIPTION
**    This function will generate an appropriate general, preformatted error
**    message.  Since this function calls MC_Error_Message, care must be
**    taken to ensure that this function isn't used until
**    MC_Library_Initialization has been called.
**
** RETURNS
**   Nothing
**
...
{
    va_list args;  /* A pointer to the variable arguments */

    /*
    ** First, we make args point to the first of the variable
    ** arguments by using va_start.  Recall that va_start is passed
    ** the last non-variable argument.
    */
    va_start( args, format );

    /*
    ** Now we print the stuff passed into us from the static arguments.
    */   
    ...
    /*
    ** Based on the error level that the user gives us, we will print a
    ** different header
    */
    switch( error_level )
    {
        ...
        case ERROR_LVL_3:
            ...
            break;
        default:
            printf( "Unknown error level.\n" );
    }
    
    /*
    ** Now, we print out the function name and line number that was given
    ** to us.  Note that the line number could also be defined just as a
    ** code number.
    */
    printf( "*************************************************************" );
    printf( "******************\n" );
    printf( "** Function:  %-30.30s                 Line:  %8d\n",
             function_name, line_number );
    
    printf( "**\n" );
    /*
    ** Then we print out the MergeCOM3 status that was given to us.
    */
    printf( "**                         MergeCOM3 status(%6d) =\n", status );
    printf( "**      %-60.60s         \n", MC_Error_Message( status ) );
    
    /*
    ** Finally, we use vprintf to print out the variable arguments based on
    ** the format string that was given to us.
    */
    printf( "**\n" );
